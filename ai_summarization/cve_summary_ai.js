...
app.post("/api/cve-summary-ai", async (req, res) => {
  try {
    ...
    const prompt = `Please generate a consistent and concise vulnerability report using the following details. 
        ...
        2) Severity Level (Critical/High): Pay particular attention to extract the Severity Level. Look for specific terms like "Critical", "High", "Medium", or "Low" in the text. If any of these terms are present, extract them accurately. Only return "" if no relevant severity information can be determined.
        ...
        Severity Level (Critical/High): [Extracted Severity Level, do not leave it blank, extract from context if necessary]
        ...
        Severity Level (Critical/High): Critical
